<div class="product-list-ctrl">
  ...
  {{-- <div class="product-ctrl-right">
    <form class="ordering" method="post" action="/">
      {{ csrf_field() }}
      <select class="product-list-orderBy" onchange="this.form.submit();">
        <option value="menu-order" name="sort" value="default">Orden Predeterminado</option>
        <option value="price-asc" name="sort" value="loHi">Ordenar por precio: bajo a alto</option>
        <option value="price-desc" name="sort" value="hiLo">Ordenar por precio: alto a bajo</option>
      </select>
    </form>
  </div> --}}
</div>
